...
def detect_languages(item):
    """JSONアイテムから存在する言語ペアを検出し、ランダムに2つ選択"""
    supported_languages = ["C", "C++", "PHP", "Java", "Python", "C#", "Go", "VB", "JavaScript", "TypeScript", "Ruby", "Swift", "Kotlin", "Rust", "Dart"]
    languages = [lang for lang in supported_languages if lang in item]
    if len(languages) < 2:
        raise ValueError(f"Expected at least 2 languages, but found {len(languages)}: {languages}")
    return random.sample(languages, 2)  # ランダムに2つ選択
