...
# -*- coding:utf-8 -*-
import sys
sys.path.append('/mnt/workspace/project/transformers/CodeTansBaselines/CodeT5/CodeT5')
import argparse
import os
from evaluator.CodeBLEU import bleu, weighted_ngram_match, syntax_match, dataflow_match
import json

def get_codebleu(refs, hyp, lang, params='0.25,0.25,0.25,0.25', naive=False, ):
    ...
    if 'json' in hyp:
        if naive:
            hypothesis = [json.loads(x)['source'] for x in open(hyp, 'r', encoding='utf-8').readlines()]
        else:
            hypothesis = [json.loads(x)['prediction'] for x in open(hyp, 'r', encoding='utf-8').readlines()]
        pre_references = [[json.loads(x)['target'] for x in open(hyp, 'r', encoding='utf-8').readlines()]]
    else:
    ...
    print('code_bleu_score', code_bleu_score)

    return code_bleu_score


def get_codebleu_list(refs, hyp, lang, params='0.25,0.25,0.25,0.25'):
    alpha, beta, gamma, theta = [float(x) for x in params.split(',')]
    pre_references = refs
    hypothesis = hyp
    # preprocess inputs
...
if __name__ == '__main__':
    ...
    code_bleu_score = get_codebleu(args.refs, args.hyp, args.lang, args.params, args.naive)
